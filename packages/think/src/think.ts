...
// wrapper: every turn's streamText call becomes an `invoke_agent` span under
// the chat turn, with `chat`/`execute_tool` children. No-op on
...
> extends Agent<Env, State, Props> {
  ...
  constructor(ctx: DurableObjectState, env: Env) {
    ...
    const startThink = async (update: UpdateAgentSpan) => {
      await withAgentSpan(
        this,
